...
class Transport:
    ...
    def sample_mf(self, x1, ratio_r_not_equal_t):
        x0 = th.randn_like(x1)
        dist_options = self.time_dist_type.split("_")
        t0, t1 = self.check_interval(self.train_eps, self.sample_eps)
        if dist_options[0] == "uniform":
            t = th.rand((x1.shape[0], 2)) * (t1 - t0) + t0
        elif dist_options[0] == "logit-normal":
            assert len(dist_options) == 3, "Logit-normal distribution must specify the mean and variance."
            mu, sigma = float(dist_options[1]), float(dist_options[2])
            assert sigma > 0, "Logit-normal distribution must have positive variance."
            t = truncated_logitnormal_sample(
                (x1.shape[0], 2), mu=mu, sigma=sigma, low=t0, high=t1
            )
        else:
            raise NotImplementedError(f"Unknown time distribution type {self.time_dist_type}")
        ...
        sorted_samples, _ = th.sort(t, dim=1)
        ...
        equal_mask = th.rand(x1.shape[0], device=x1.device) < fraction_equal
        # Apply the mask: where equal_mask is True, set r=t (replace)
        r = th.where(equal_mask, t, r)
        
        return r, t, x0, x1
    

    def training_losses_mft(self, model, x1, model_kwargs=None,
        ratio_r_not_equal_t=1.0/4,
        ...
        unconditional_mask = th.zeros(batch_size, dtype=th.bool, device=device)
        
        if model_kwargs.get('y') is not None and self.label_dropout_prob > 0:
            y = model_kwargs['y'].clone()  
            batch_size = y.shape[0]
            num_classes = 1000 # model.module.num_classes
            dropout_mask = th.rand(batch_size, device=y.device) < self.label_dropout_prob
            
            y[dropout_mask] = num_classes
            model_kwargs['y'] = y
            unconditional_mask = dropout_mask  # Used for unconditional velocity computation
        
        r, t, noises, images = self.sample_mf(x1, ratio_r_not_equal_t)

        # Calculate interpolation and z_t
        alpha_t, sigma_t, d_alpha_t, d_sigma_t = 1 - t.view(-1, 1, 1, 1), t.view(-1, 1, 1, 1), -1, 1
        z_t = alpha_t * images + sigma_t * noises
        v_t = d_alpha_t * images + d_sigma_t * noises

        time_diff = (t - r).view(-1, 1, 1, 1)
                
        u_target = th.zeros_like(v_t)

        # print(" No labels or no CFG applicable samples, use standard JVP")
        # No labels or no CFG applicable samples, use standard JVP
        z_p = z_t + self.delta_t * v_t
        z_m = z_t - self.delta_t * v_t
        t_p = t + self.delta_t
        t_m = t - self.delta_t
        
        z_all = th.cat([z_t, z_p, z_m])
        t_all = th.cat([t, t_p, t_m])
        r_all = th.cat([r, r, r])
        y = model_kwargs['y']
        y_all = th.cat([y, y, y])
        u_all = model(z_all, r_all, t_all, y=y_all)
        u, u_p, u_m = th.chunk(u_all, 3, dim=0)

        # dudt = fn_current(z_p, r, t_p) - fn_current(z_m, r, t_m)
        dudt = u_p - u_m
        dudt /= (2 * self.delta_t)
        ...
        loss_mid = th.sum((error**2).reshape(error.shape[0],-1), dim=-1)
        ...
        loss_mean_ref = th.mean((error**2))
        
        terms = {}
        terms['pred'] = u
        terms['loss'] = loss
        terms['loss_ref'] = loss_mean_ref
        return terms


    def training_losses_mfd(self, model, model_teacher, x1, model_kwargs=None,
        ratio_r_not_equal_t=1.0/4,
        adaptive_p=1.0,               # Power param for adaptive weighting
        label_dropout_prob=0.1,       # Drop out label
        delta_t=5e-3,
        ...
        with th.no_grad(): 
            v_t = model_teacher(z_t, t, y=model_kwargs.get('y'))
            v_t_1point = d_alpha_t * images + d_sigma_t * noises
    ...
    def training_losses_cmt_v2(self, model, model_teacher, sample_fn, x1, model_kwargs=None, label_dropout_prob=0.1, num_samples=64):
        self.label_dropout_prob=label_dropout_prob
        ...
        with th.no_grad(): samples = sample_fn(z, model_teacher.forward, **model_kwargs) # [-1] is clean
        ode_steps = len(samples)
        t_all = np.linspace(0, 1, ode_steps)
        t, r = [], []
        x_src = []
        x_tgt = []
        y_src = []
        for i in range(1, ode_steps):
            for j in range(i):
                x_src_temp = samples[ode_steps - 1 - i]
                x_src.append(x_src_temp)
                x_tgt_temp = x_src_temp - samples[ode_steps - 1 - j]
                x_tgt_temp = x_tgt_temp / (t_all[i] - t_all[j])
                x_tgt.append(x_tgt_temp)
                t.extend([t_all[i]] * batch_size)
                r.extend([t_all[j]] * batch_size)
                y_src.append(model_kwargs['y'])

        x_src = th.cat(x_src, 0)
        x_tgt = th.cat(x_tgt, 0)
        t = th.tensor(t).to(samples[0].device).to(x_src.dtype)
        r = th.tensor(r).to(samples[0].device).to(x_src.dtype)
        y_src = th.cat(y_src, 0)

        indices = th.randint(0, x_src.shape[0], (num_samples,))
        x_src = x_src[indices]
        x_tgt = x_tgt[indices]
        t = t[indices]
        r = r[indices]
        ...
        # print(x_src.shape, r.shape, t.shape)

        u = model(x_src, r, t, **model_kwargs)
                
        error = u - x_tgt.detach()
        loss = th.mean((error**2).reshape(error.shape[0],-1), dim=-1)
        loss_mean_ref = th.mean((error**2))
    ...
    def training_losses_cmt_512(self, model, model_teacher, sample_fn, x1, model_kwargs=None, label_dropout_prob=0.1):
        self.label_dropout_prob=label_dropout_prob
        ...
        indices = th.randint(0, x_src.shape[0], (8,))
        x_src = x_src[indices]
        x_tgt = x_tgt[indices]
        t = t[indices]
        r = r[indices]
