...
def main(args):
    ...
    if mode == "ODE":
        detail_components = [mode, str(args.mf_step), str(sampling_method), args.precision]
    else:
        ...
        detail_components = [mode, str(args.mf_step), str(sampling_method), str(diffusion_form), str(last_step), str(last_step_size), args.precision]
    ...
    if args.only_npz:
        print("You are creatinng the npz!!!!!!!!!")
        dist.barrier()
        if rank == 0:
    ...
    t05 = torch.zeros(n, device=device) + 0.5
    t13, t23 = torch.zeros(n, device=device) + 1 / 3, torch.zeros(n, device=device) + 2 / 3
    t14, t34 = torch.zeros(n, device=device) + 1 / 4, torch.zeros(n, device=device) + 3 / 4

    if not args.only_npz:
        print("You are sampling the image!!!!!!!!!")
        for step_idx in pbar:
            with autocast(**autocast_kwargs):
                ...
                if args.mf_step == 1:
                    samples = z - model_fn(z, t0, t1, y=y)
                elif args.mf_step == 2:
                    z = z - 0.5 * model_fn(z, t05, t1, y=y)
                    samples = z - 0.5 * model_fn(z, t0, t05, y=y)
                elif args.mf_step == 3:
                    z = z - model_fn(z, t23, t1, y=y) / 3
                    z = z - model_fn(z, t13, t23, y=y) / 3
                    samples = z - model_fn(z, t0, t13, y=y) / 3
                elif args.mf_step == 4:
                    z = z - model_fn(z, t34, t1, y=y) / 4
                    z = z - model_fn(z, t05, t34, y=y) / 4
                    z = z - model_fn(z, t14, t05, y=y) / 4
                    samples = z - model_fn(z, t0, t14, y=y) / 4
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mf-step", type=int, default=1)
    ...
    parser.add_argument("--only-npz", type=int, default=0)
