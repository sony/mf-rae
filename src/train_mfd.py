...
from stage2.models.DDT import DiTwDDTHead, DiTwDDTHead_MF
...
def main(args):
    ...
    model = DiTwDDTHead_MF(
        input_size = 16,
        patch_size = 1,
        in_channels = 768,
        hidden_size = [1152, 2048], # [1152, 2048], (XL) # [768, 2048], (B) # [384, 2048], (S)
        depth = [28, 2], # [28, 2], # [12, 2], (B, S)
        num_heads = [16, 16], # [16, 16], # [12, 16], # [6, 16], (S)
        mlp_ratio = 4.0,
    ...
    if args.ckpt_teacher is not None:
        logger.info(f"Init MF via pre-trained SiT...")
        checkpoint = torch.load(args.ckpt_teacher, map_location="cpu")
    ...
    if args.ckpt_cmt is not None:
        logger.info(f"Init MF via CMT...")
        checkpoint = torch.load(args.ckpt_cmt, map_location="cpu")
        if "ema" in checkpoint:
            checkpoint = checkpoint['ema']
        model.load_state_dict(checkpoint, strict=False)

    logger.info(f"Creating EMA with Same Init as MF...")
    ...
    if args.ckpt is not None:
        logger.info(f"Resume previous ckpt...")
    ...
    ys = torch.randint(num_classes, size=(64,), device=device)
    using_cfg = guidance_scale > 1.0
    n = ys.size(0)
    zs = torch.randn(n, *latent_size, device=device, dtype=latent_dtype)
    t1, t0 = torch.zeros(n, device=device) + 1, torch.zeros(n, device=device)
    ...
    for epoch in range(epochs):
        ...
        for x, y in loader:
            ...
            with autocast(**autocast_kwargs):
                loss_all = transport.training_losses_mfd(model, model_teacher, x, model_kwargs)
                loss_tensor = loss_all["loss"].mean() 
                loss_ref = loss_all['loss_ref']
            step_loss_accum += loss_ref.item()
            (loss_tensor / grad_accum_steps).backward()
            accum_counter += 1

            if torch.isnan(loss_tensor) or torch.isinf(loss_tensor):
                logger.info(f"NaN detected. Stopping training.")
                raise SystemExit  # or use break/return depending on your code structure
            ...
            if train_steps % ckpt_every == 0 and train_steps > 0:
                if rank == 0:
                    ...
                    checkpoint_path = f"{checkpoint_dir}/ema.pt"
                    torch.save(checkpoint, checkpoint_path)
                    logger.info(f"Saved ema to {checkpoint_path}")
                dist.barrier()
            ...
            if train_steps % sample_every == 0 or train_steps == 1:
                ...
                with torch.no_grad():
                    with autocast(**autocast_kwargs):
                        samples = zs - ema(zs, t0, t1, y=ys)
                    dist.barrier()
                    samples = rae.decode(samples.to(torch.float32)).clamp(0, 1)
                    img_path = f"{checkpoint_dir}/{train_steps:07d}.png"
                    if rank == 0: save_image(samples, img_path, nrow=8)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_cmt", type=str, default='/path/to/cmt.pt', help="CMT checkpoint")
    parser.add_argument("--ckpt_teacher", type=str, default='models/DiTs/Dinov2/wReg_base/ImageNet256/DiTDH-XL/stage2_model.pt', help="Compulsory teacher checkpoint path for distillation.")
